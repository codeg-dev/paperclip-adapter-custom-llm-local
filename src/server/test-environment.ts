...
export async function testEnvironment(ctx: AdapterEnvironmentTestContext): Promise<AdapterEnvironmentTestResult> {
  ...
  checks.push({
    code: "custom_llm_local_config_valid",
    level: "info",
    message: `Config valid: transport=${config.transport}, baseUrl=${config.baseUrl}, model=${config.model}`,
  });
  ...
  if (config.apiKeyEnv) {
    ...
    if (!keyValue) {
      ...
      checks.push({
        code: "custom_llm_local_api_key_present",
        level: "info",
        message: `apiKeyEnv "${config.apiKeyEnv}" is set`,
      });
    }
  } else {
    checks.push({
      code: "custom_llm_local_no_api_key_env",
      level: "info",
      message: "No apiKeyEnv configured; endpoint will be called without Authorization or x-api-key header",
    });
  }
